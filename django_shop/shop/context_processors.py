...
def shop(request):
    """shop() определяет переменную categories, которая содержит список всех объектов модели Category.
    Эта переменная добавляется в контекст и становится доступной во всех шаблонах,
